...
public class SwissTournament16TeamsTest extends AbstractTestNGSpringContextTests {
	...
	public void addParticipants() {
		for (int i = 0; i < MEMBERS * TEAMS; i++) {
			this.participantController.create(new ParticipantDTO(String.format("S16-%04d", i),
	...
	public void createAndAdvanceSwissRoundsWithoutByes() {
		...
		for (int level = 0; level < ROUNDS; level++) {
			...
			final List<Fight> fightsInRound = roundGroups.stream().flatMap(group -> group.getFights().stream())
					.toList();
			...
			final int minimumCrossBracketPairings = SwissTestAssertions.getMinimumCrossBracketPairings(winsBeforeRound,
					null, "without byes at level " + roundLevel);
			final int actualCrossBracketPairings = SwissTestAssertions.countCrossBracketPairings(fightsInRound,
					winsBeforeRound);
			...
			Assert.assertTrue(actualCrossBracketPairings <= minimumCrossBracketPairings + 2,
					"Cross-bracket pairings at level " + roundLevel + " should stay near minimum. " + "actual="
							+ actualCrossBracketPairings + ", min=" + minimumCrossBracketPairings);
			...
			Assert.assertEquals(this.groupController.getGroups(this.tournamentDTO, roundLevel).stream()
					.mapToLong(group -> group.getFights().size()).sum(), FIGHTS_PER_ROUND);
	...
	public void checkGroupsPerSwissRound() {
		for (int level = 0; level < ROUNDS; level++) {
			...
			Assert.assertEquals(uniqueTeams.size(), TEAMS, "No team can appear in multiple groups at level " + level);

			Assert.assertEquals(roundGroups.stream().mapToLong(group -> group.getFights().size()).sum(),
					FIGHTS_PER_ROUND);
		}
	}
	...
	public void checkFinalRanking() {
		...
		Assert.assertTrue(
				ranking.stream().allMatch(score -> score.getWonFights() >= 0 && score.getWonFights() <= ROUNDS),
