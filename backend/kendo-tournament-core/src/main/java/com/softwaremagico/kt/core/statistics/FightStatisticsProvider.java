...
public class FightStatisticsProvider {
    ...
    public FightStatisticsDTO calculate(TournamentType tournamentType, boolean maximizeFights, Collection<TeamDTO> teams) {
        if (tournamentType == null || teams == null || teams.size() < 2) {
            return null;
        }
        switch (tournamentType) {
            case LEAGUE:
                return calculateLeagueStatistics(teams);
            case LOOP:
                return calculateLoopStatistics(maximizeFights, teams);
            case CUSTOMIZED:
    ...
    private FightStatisticsDTO calculateLeagueStatistics(Collection<TeamDTO> teams) {
        ...
        fightStatisticsDTO.setDuelNumber(getDuels(fightStatisticsDTO.getFightsByTeam(), teams));
    ...
    private FightStatisticsDTO calculateLoopStatistics(boolean maximizeFights, Collection<TeamDTO> teams) {
        final FightStatisticsDTO fightStatisticsDTO = new FightStatisticsDTO();
        fightStatisticsDTO.setFightNumber((teams.size() * teams.size() - 1) / 2);
        if (maximizeFights) {
    ...
    private int getDuels(int fightByTeam, Collection<TeamDTO> teams) {
        ...
        teams.forEach(teamDTO -> {
            ...
            missingMembers.getAndIncrement();
        });
