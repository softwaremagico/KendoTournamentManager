package com.softwaremagico.kt.persistence.encryption;

/*-
 * #%L
 * Kendo Tournament Manager (Persistence)
 * %%
 * Copyright (C) 2021 - 2026 Softwaremagico
 * %%
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with this program.  If not, see <http://www.gnu.org/licenses/>.
 * #L%
 */

import org.testng.Assert;
import org.testng.annotations.BeforeClass;
import org.testng.annotations.Test;

@Test(groups = "cryptoConverters")
public class CryptoConvertersTests {

	private static final String ENCRYPTION_CODE = "myEncryptionCode";
	private static final String PUBLIC_KEY = "";
	private static final String PRIVATE_KEY = "";

	private FloatCryptoConverter floatConverter;
	private BooleanCryptoConverter booleanConverter;

	@BeforeClass
	public void setup() {
		new KeyProperty(ENCRYPTION_CODE, PUBLIC_KEY, PRIVATE_KEY);
		this.floatConverter = new FloatCryptoConverter();
		this.booleanConverter = new BooleanCryptoConverter();
	}

	// FloatCryptoConverter tests
	@Test
	public void testFloatConverterWithNull() {
		final String encrypted = this.floatConverter.convertToDatabaseColumn(null);
		Assert.assertNull(encrypted);
		Assert.assertNotEquals(encrypted, "");
	}

	@Test
	public void testFloatConverterWithValue() {
		final Float originalValue = 123.45f;
		final String encrypted = this.floatConverter.convertToDatabaseColumn(originalValue);
		Assert.assertNotNull(encrypted);
		Assert.assertFalse(encrypted.isEmpty());
	}

	@Test
	public void testFloatConverterRoundTrip() {
		final float originalValue = 987.65f;
		final String encrypted = this.floatConverter.convertToDatabaseColumn(originalValue);
		final Float decrypted = this.floatConverter.convertToEntityAttribute(encrypted);
		Assert.assertNotNull(decrypted);
		Assert.assertEquals(decrypted, originalValue, 0.001f);
		Assert.assertNotEquals(decrypted, 0f);
		Assert.assertNotEquals(decrypted, -originalValue);
	}

	@Test
	public void testFloatConverterWithZero() {
		final Float originalValue = 0f;
		final String encrypted = this.floatConverter.convertToDatabaseColumn(originalValue);
		final Float decrypted = this.floatConverter.convertToEntityAttribute(encrypted);
		Assert.assertNotNull(decrypted);
		Assert.assertEquals(decrypted, originalValue);
		Assert.assertEquals(decrypted, 0f);
		Assert.assertNotEquals(decrypted, 1f);
	}

	@Test
	public void testFloatConverterWithNegativeValue() {
		final Float originalValue = -456.78f;
		final String encrypted = this.floatConverter.convertToDatabaseColumn(originalValue);
		final Float decrypted = this.floatConverter.convertToEntityAttribute(encrypted);
		Assert.assertNotNull(decrypted);
		Assert.assertEquals(decrypted, originalValue, 0.001f);
		Assert.assertTrue(decrypted < 0);
		Assert.assertFalse(decrypted > 0);
	}

	@Test
	public void testFloatConverterWithEmptyString() {
		final Float result = this.floatConverter.convertToEntityAttribute("");
		Assert.assertNull(result);
	}

	@Test
	public void testFloatConverterWithNullString() {
		final Float result = this.floatConverter.convertToEntityAttribute(null);
		Assert.assertNull(result);
	}

	@Test
	public void testFloatConverterWithInvalidString() {
		final Float result = this.floatConverter.convertToEntityAttribute("not_a_float");
		Assert.assertNull(result);
	}

	// BooleanCryptoConverter tests
	@Test
	public void testBooleanConverterWithNull() {
		final String encrypted = this.booleanConverter.convertToDatabaseColumn(null);
		Assert.assertNull(encrypted);
		Assert.assertNotEquals(encrypted, "");
	}

	@Test
	public void testBooleanConverterWithTrue() {
		final Boolean originalValue = true;
		final String encrypted = this.booleanConverter.convertToDatabaseColumn(originalValue);
		Assert.assertNotNull(encrypted);
		Assert.assertFalse(encrypted.isEmpty());
	}

	@Test
	public void testBooleanConverterWithFalse() {
		final Boolean originalValue = false;
		final String encrypted = this.booleanConverter.convertToDatabaseColumn(originalValue);
		Assert.assertNotNull(encrypted);
		Assert.assertFalse(encrypted.isEmpty());
	}

	@Test
	public void testBooleanConverterRoundTripTrue() {
		final Boolean originalValue = true;
		final String encrypted = this.booleanConverter.convertToDatabaseColumn(originalValue);
		final Boolean decrypted = this.booleanConverter.convertToEntityAttribute(encrypted);
		Assert.assertNotNull(decrypted);
		Assert.assertEquals(decrypted, originalValue);
		Assert.assertTrue(decrypted);
		Assert.assertTrue(decrypted);
	}

	@Test
	public void testBooleanConverterRoundTripFalse() {
		final Boolean originalValue = false;
		final String encrypted = this.booleanConverter.convertToDatabaseColumn(originalValue);
		final Boolean decrypted = this.booleanConverter.convertToEntityAttribute(encrypted);
		Assert.assertNotNull(decrypted);
		Assert.assertEquals(decrypted, originalValue);
		Assert.assertFalse(decrypted);
		Assert.assertFalse(decrypted);
	}

	@Test
	public void testBooleanConverterWithEmptyString() {
		final Boolean result = this.booleanConverter.convertToEntityAttribute("");
		Assert.assertFalse(result);
	}

	@Test
	public void testBooleanConverterWithNullString() {
		final Boolean result = this.booleanConverter.convertToEntityAttribute(null);
		Assert.assertFalse(result);
	}

	@Test
	public void testBooleanConverterDifferentEncryptions() {
		final Boolean originalValue1 = true;
		final Boolean originalValue2 = false;
		final String encrypted1 = this.booleanConverter.convertToDatabaseColumn(originalValue1);
		final String encrypted2 = this.booleanConverter.convertToDatabaseColumn(originalValue2);
		Assert.assertNotNull(encrypted1);
		Assert.assertNotNull(encrypted2);
		Assert.assertNotEquals(encrypted1, encrypted2);
		Assert.assertNotEquals(encrypted2, encrypted1);
	}
}
