...
public class AuthFirstLoginIntegrationTest extends AbstractTestNGSpringContextTests {
	...
	private static final String TENANT_NAME = "Legacy organization";
	...
	public void testFirstLoginCreatesAdminUserAndTenantAndCanLoginAgain() throws Exception {
		...
		final MvcResult firstLoginResult = this.mockMvc
		...
		Assert.assertNotNull(tenantRepository.findByNameAndActiveTrue(TENANT_NAME),
				"Default tenant 'Legacy organization' should exist");

		// Verify that the user was created
		Assert.assertEquals(authenticatedUserRepository.count(), 1L, "Admin user should be created");
		Assert.assertNotNull(authenticatedUserRepository.findByUsername(FIRST_ADMIN_USERNAME),
				"Admin user should be created with provided username");
		...
		secondLoginRequest.setTenant(TENANT_NAME);

		final MvcResult secondLoginResult = this.mockMvc
				.perform(post("/auth/public/login")
						.contentType(MediaType.APPLICATION_JSON)
						.content(toJson(secondLoginRequest))
						.with(csrf()))
				.andExpect(MockMvcResultMatchers.status().isOk())
				.andExpect(MockMvcResultMatchers.header().exists(HttpHeaders.AUTHORIZATION))
				.andReturn();
